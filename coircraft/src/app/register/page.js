...
export default function RegisterPage() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      if (accounts.find((a) => a.email === form.email)) { setError("Email already registered."); setLoading(false); return; }
  ...
  return (
    <>
      <Navbar />
      <main style={{ background: "#FFFEF5", minHeight: "100vh", fontFamily: "var(--font-body)", padding: "64px 16px" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 560, margin: "0 auto" }}>
          <div style={{ background: "#fff", borderRadius: 28, boxShadow: "0 20px 60px rgba(14,32,17,0.12)", padding: "48px 44px" }}>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
                {fields.map(({ key, label, type, placeholder, span }) => (
                  <div key={key} style={{ gridColumn: span === 2 ? "1 / -1" : "auto" }}>
                    <label style={{ display: "block", fontSize: 13, fontWeight: 700, color: "#0E2011", marginBottom: 8 }}>{label}</label>
                    <div style={{ position: "relative" }}>
                      <input
                        type={type === "password" && key === "password" ? (showPass ? "text" : "password") : type === "password" ? (showPass ? "text" : "password") : type}
              ...
              <button type="submit" disabled={loading}
                style={{ width: "100%", background: loading ? "#ccc" : "linear-gradient(135deg,#A8FF3E,#5BCC1C)", color: "#0E2011", border: "none", padding: "15px", borderRadius: 50, fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", boxShadow: loading ? "none" : "0 4px 20px rgba(168,255,62,0.4)", marginTop: 24, transition: "all 0.25s" }}>
