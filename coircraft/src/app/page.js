  ...
  return (
    <section className="tk-hero" style={{ background: "linear-gradient(140deg,#0E2011 0%,#1A472A 50%,#0E2011 100%)", position: "relative", overflow: "hidden", textAlign: "center" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 700, margin: "0 auto" }}>
        ...
        <div className="anim-fadeUp delay-4" style={{ display: "flex", gap: "clamp(16px,5vw,44px)", justifyContent: "center", flexWrap: "wrap" }}>
          {[["🌿","Natural","Zero synthetic"],["🇵🇭","Local","Made in PH"],["♻️","Zero Waste","Biodegradable"]].map(([icon,title,sub]) => (
            <div key={title} style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ background: "rgba(168,255,62,0.12)", borderRadius: 12, width: 42, height: 42, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>{icon}</div>
              <div>
...
function UserHero({ user }) {
  return (
    <section className="tk-hero" style={{ background: "linear-gradient(140deg,#0E2011 0%,#1A472A 60%,#0E2011 100%)", position: "relative", overflow: "hidden" }}>
      <div className="tk-glow tk-glow-lime" style={{ width: 500, height: 500, top: "-30%", right: "-10%" }} />
      <div style={{ position: "absolute", right: "3%", bottom: "-5%", fontSize: "clamp(100px,15vw,220px)", opacity: 0.05, userSelect: "none" }}>🥥</div>

      <div style={{ position: "relative", zIndex: 1, maxWidth: 720 }}>
        <div className="tk-pill anim-fadeUp" style={{ marginBottom: 18, display: "inline-flex" }}>
          <span className="tk-pill-dot" /> Welcome back, {user.name.split(" ")[0]}!
        </div>
        <h1 className="tk-h1 anim-fadeUp delay-1" style={{ color: "#fff", margin: "0 0 20px", maxWidth: 600 }}>
          Ready to discover<br />
          <span style={{ background: "linear-gradient(90deg,#A8FF3E,#6BCC1C)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
            new products?
          </span>
        </h1>
        <div className="anim-fadeUp delay-2" style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
  ...
  return (
    <>
      ...
      <main style={{ background: "var(--tk-bg)", fontFamily: "var(--font-body)", overflowX: "hidden" }}>
        ...
        {user ? <UserHero user={user} /> : <GuestHero />}
          ...
          <section className="tk-section" style={{ paddingTop: 32, paddingBottom: 32 }}>
            <div className="tk-container">
              <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 4, scrollbarWidth: "none" }}>
                ...
                {CATS.map(({ label, icon }) => (
                  <Link key={label} href={`/products?cat=${label}`}
              ...
              <div style={{ background: "linear-gradient(135deg,#0E2011,#1A472A)", borderRadius: 28, padding: "clamp(28px,6vw,56px) clamp(20px,5vw,64px)", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 40, alignItems: "center", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ position: "relative", zIndex: 1 }}>
                  <h2 className="tk-h2" style={{ color: "#fff", margin: "0 0 16px" }}>
                    Why Choose<br />
                    <span style={{ color: "#A8FF3E" }}>Philippine Coir?</span>
                  </h2>
            ...
            <div className="tk-container">
              ...
              <style>{`
                ...
                @media (min-width: 900px) {
                  .testimonial-grid { grid-template-columns: 1fr 1fr 1fr; gap: 20px; }
                }
              `}</style>
